...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $id_gambar = 2; // ID gambar yang akan diupdate
  ...
  if (isset($_FILES['gambar'])) {
    ...
    if (!empty($uploadedFiles)) {
      ...
      echo '<script>alert("' . $message . '"); window.location.href = "editkosPermata.php";</script>';
    } else {
      // Menampilkan pesan error menggunakan pop-up
      $errorMessage = "Terjadi kesalahan dalam mengupdate gambar: " . implode(", ", $uploadErrors);
      echo '<script>alert("' . $errorMessage . '"); window.location.href = "editkosPermata.php";</script>';
    }
  } else {
    // Menampilkan pesan jika file gambar tidak ditemukan
    echo '<script>alert("File gambar tidak ditemukan. Mohon unggah file gambar."); window.location.href = "editkosPermata.php";</script>';
  }
