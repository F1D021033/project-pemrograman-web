...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $id_gambar = 1; // ID gambar yang akan diupdate
  ...
  if (isset($_FILES['gambar'])) {
    ...
    if (!empty($uploadedFiles)) {
      ...
      echo '<script>alert("' . $message . '"); window.location.href = "editkosNusa.php";</script>';
    } else {
      // Menampilkan pesan error menggunakan pop-up
      $errorMessage = "Terjadi kesalahan dalam mengupdate gambar: " . implode(", ", $uploadErrors);
      echo '<script>alert("' . $errorMessage . '"); window.location.href = "editkosNusa.php";</script>';
    }
  } else {
    // Menampilkan pesan jika file gambar tidak ditemukan
    echo '<script>alert("File gambar tidak ditemukan. Mohon unggah file gambar."); window.location.href = "editkosNusa.php";</script>';
  }
